...
let marcasNumber = 0;
let marcasOrder = data_1.marcas.sort((a, b) => a.marca.localeCompare(b.marca)).map(marca => marca.marca);
for (const marca of data_1.marcas) {
    marcasNumber++;
}
;
...
for (const inventario of data_1.inventory) {
    cantidadExistencias++;
}
;
let existencia = data_1.inventory[0];
for (const existencias of data_1.inventory) {
...
let heroCity = {};
for (const heroe of data_1.heroes) {
